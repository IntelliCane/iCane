...
// endpoint for therapist
// TBI: there's a better way to do this using express internal routing
const tApp = express();

// TBI: rewrite by chaining thenables together instead of nesting

// validation to make sure caller is admin
const validateFirebaseIdTokenAdmin = (req, res, next) => {
...
// validation to make sure caller is therapist
const validateFirebaseIdTokenTherapist = (req, res, next) => {
	console.log('Check if request is authorized with Firebase ID token');
	...
	admin.auth().verifyIdToken(idToken).then(decodedIdToken => {
		...
		ref.once("value", function(data) {
  			var role = data.val();
  			if ("Therapist" !== role ){
  				console.error('You do not have therapist privs');
				res.status(403).send({status:'Unauthorized'});
				return;
  			}
		}, function( error ){
...
app.use(validateFirebaseIdTokenAdmin);

tApp.use(cors);
tApp.use(cookieParser);
tApp.use(validateFirebaseIdTokenTherapist);
...
tApp.post('/therapist/addPatient', (req, res) => {
	...
	admin.auth().createUser({
		...
		patientUid = userRecord.uid;
		console.log("Successfully created new auth user: ", patientUid ); 

	  	// Push the new message into the Realtime Database using the Firebase Admin SDK.
  		admin.database().ref('/Users').child("Patient").child(patientUid).set({
			"fname": fname,
			"lname": lname,
			"email": email
  		}).then(function(){

  			// they've been added to auth, send password
  			// use nodemailer to send with new password
  			// ...

  			// add them to the roles node
    		admin.database().ref('/Roles').child(patientUid).set("Patient").then(function(){
    			// automatically add patient to that therapists patient list
    			admin.database().ref('/Users').child('Therapist').child(therapistUid).child('Patients').child(patientUid).set({
    				"fname": fname,
					"lname": lname,
					"email": email
    			}).then(function(){
    				...
    				console.log("Problem adding patient to therapists list.");
    				res.status(500).send({status:error.message});
    				return;
    			});
    		}).catch(function(error){
...
exports.tApp = functions.https.onRequest(tApp);
